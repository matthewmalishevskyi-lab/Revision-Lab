...
export function circleFree(w: World, x: number, y: number, r: number): boolean {
  const x0 = Math.floor(x - r), x1 = Math.floor(x + r), y0 = Math.floor(y - r), y1 = Math.floor(y + r);
  for (let cy = y0; cy <= y1; cy++) for (let cx = x0; cx <= x1; cx++) {
    if (!blocked(w, cx, cy)) continue;
    const nx = Math.max(cx, Math.min(x, cx + 1)), ny = Math.max(cy, Math.min(y, cy + 1));
...
export function moveCircle(w: World, x: number, y: number, dx: number, dy: number, r: number): [number, number] {
  const n = Math.max(1, Math.ceil(Math.hypot(dx, dy) / MAX_STEP));
  for (let i = 0; i < n; i++) [x, y] = pushOut(w, x + dx / n, y + dy / n, r);
  return [x, y];
}

/** Is the straight path from a to b clear for a circle? (Used when something throws you.) */
export function pathClear(w: World, ax: number, ay: number, bx: number, by: number, r: number): boolean {
  const n = Math.max(1, Math.ceil(Math.hypot(bx - ax, by - ay) / 0.1));
  for (let i = 1; i <= n; i++) if (!circleFree(w, ax + ((bx - ax) * i) / n, ay + ((by - ay) * i) / n, r)) return false;
  return true;
}
